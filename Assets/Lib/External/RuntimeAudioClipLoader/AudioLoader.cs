...
namespace RuntimeAudioClipLoader
{
    ...
    public class AudioLoader
    {
        ...
        public List<AudioLoaderConfig> MTConfigs;
        ...
        public AudioDataReader[] MTReaders;

        //AudioProcessingManager APM;
        ...
        {
            this.Config = config;
            MTConfigs = new List<AudioLoaderConfig>();
            //APM = new GameObject().AddComponent<AudioProcessingManager>();
            CreateReader();
        ...
        AudioDataReader CreateNewReader(int i)
        {
            try
            {
                AudioDataReader newReader = new AudioDataReader(MTConfigs[i].DataStream, MTConfigs[i].AudioFormat, MTConfigs[i].PreferredDecoder);
                SamplesCount = (int)(newReader.Length / (long)(newReader.WaveFormat.BitsPerSample / 8));
                Channels = newReader.WaveFormat.Channels;
                SampleRate = newReader.WaveFormat.SampleRate;
                UsedDecoder = newReader.UsedDecoder;
                return newReader;
            }
            catch (Exception exception)
            {
            ...
            try
            {
                switch (Config.LoadMethod)
                {
                    case LoadMethod.AllPartsInBackgroundThread:
                        ...
                        MTReaders = new AudioDataReader[SplitMultiplier];
                        chunkSize = SamplesCount / SplitMultiplier;

                        for (int i = 0; i < SplitMultiplier; i++)
                        {
                            int offset = i;
                            MTReaders[offset] = CreateNewReader(offset);


                            Task task = MTConfigs[offset].pcQ.EnqueueTask(() => LoadSegmentInBackgroundThread(offset), Config.pcQ.cancelSource.Token);

                            //NativeArray<int> startPointForSegment = new NativeArray<int>(1, Allocator.TempJob);
                            //NativeArray<float> dataToSet = new NativeArray<float>(1, Allocator.TempJob);
                            //
                            //var job = new LoadSegmentJob()
                            //{
                            //	config = MTConfigs[offset],
                            //	reader = MTReaders[offset],
                            //	index = offset,
                            //	SamplesCount = SamplesCount,
                            //	channels = Channels,
                            //	chunkSize = chunkSize,
                            //	setDataAtOffset = startPointForSegment,
                            //	SamplesLoaded = SamplesLoaded,
                            //	SplitMultiplier = SplitMultiplier,
                            //	dataToSet = dataToSet,
                            //};

                            //job.ScheduleParallel(m_Query, 1, Dependency);


                            //Config.BackgroundThreadRunner.Enqueue(() => LoadInBackgroundThread(offset));
                        }
        ...
        void LoadInBackgroundThread()
        {
            try
            {
                ...
                while (currentOffset < SamplesCount)
                {
                    if (currentOffset + currentDataLength >= SamplesCount)
                        currentDataLength = SamplesCount - currentOffset; // this is last part, will read less samples
                    UnityEngine.Debug.Log(SamplesCount + " : " + currentDataLength);
                    currentData = Config.FloatArrayPool.Rent(currentDataLength);
        ...
        void LoadSegmentInBackgroundThread(int index)
        {
            System.Diagnostics.Stopwatch loadTimer = Stopwatch.StartNew();
            if (segTimer == null)
            { segTimer = System.Diagnostics.Stopwatch.StartNew(); }

            try
            {
                //ClampToMultiplierOf(ref SplitMultiplier, Channels);
                //int chunkSize = SamplesCount / SplitMultiplier; 
                var loadingHasAborted = false;
                var currentDataLength = MTConfigs[index].LoadBufferSize;  //Sets to max buffer size
                if (currentDataLength > SamplesCount) currentDataLength = SamplesCount; //Restricts currentDataLength from being larger than samples available 
                ClampToMultiplierOf(ref currentDataLength, Channels); //Makes sures there is a corrent number of samples for the number of channels
                                                                      //UnityEngine.Debug.Log(index + " : " + chunkSize + " : " + SamplesCount);

                float[] currentData; //Create array to store data
                int LoadPoint = chunkSize * index; //Create offset
                List<float> totalData = new List<float>();
                MTReaders[index].Position = (MTReaders[index].Length / SplitMultiplier) * index;
                var raiseProgressChangedStopwatch = Stopwatch.StartNew(); //Create stopwatch
                                                                          //UnityEngine.Debug.Log(LoadPoint + " : " + MTReaders[index].Position + " : " + MTReaders[index].Length);

                while (LoadPoint < (chunkSize * (index + 1))) //So long as loadPoint < total samples
                {
                    if (LoadPoint + currentDataLength >= (chunkSize * (index + 1))) //If loadPoint and currentBinSize are greater than the total samples
                        currentDataLength = (chunkSize * (index + 1)) - LoadPoint; // this is last part, will read less samples //Limit currentBinSize

                    currentData = MTConfigs[index].FloatArrayPool.Rent(currentDataLength); //Initialize storage space for samples to be read into

                    //MTReaders[index].b
                    //UnityEngine.Debug.Log(currentData.Length + " : " + LoadPoint + " : " + currentDataLength);
                    MTReaders[index].Read(currentData, 0, currentDataLength); //read data into 1 starting at 2 for 3 samples

                    if (loadingHasAborted)
                    {
                        MTConfigs[index].FloatArrayPool.Return(currentData);
                        MyOnLoadingAborted();
                        return;
                    }

                    var dataToSet = currentData;
                    totalData.AddRange(currentData);

                    var setDataAtOffset = LoadPoint / Channels;

                    MTConfigs[index].UnityMainThreadRunner.Enqueue(() =>
                    {
                        if (ShouldAbortLoading) // here we are comparing UnityEngine.AudioClip to null, in older Unity version this is possible only on main Unity's thread
                            loadingHasAborted = true;
                        else
                            AudioClip.SetData(dataToSet, setDataAtOffset);
                        MTConfigs[index].FloatArrayPool.Return(dataToSet);
                    });

                    LoadPoint += currentDataLength;
                    SamplesLoaded = LoadPoint;
                    //UnityEngine.Debug.Log(index + " : " + currentOffset + " : " + SamplesLoaded);
                    //UnityEngine.Debug.Log(MTReaders[index].Position + " : " + MTReaders[index].Length);
                ...
                MyOnLoadingDone();
                //if (PerformanceTestHandler.instance)
                //{
                //    MTConfigs[index].UnityMainThreadRunner.Enqueue(() => FileImportPerformanceTest.instance.Stage3Time[index].text = Convert.ToSingle(loadTimer.Elapsed.TotalSeconds).ToString());
                //}
            }
            catch (Exception exception)
            {
        ...
        int completedLoading = 0;

        void MyOnLoadingDone()
        {
            if (completedLoading == SplitMultiplier - 1)
            {
                //if (PerformanceTestHandler.instance)
                //{
                //    MTConfigs[1].UnityMainThreadRunner.Enqueue(() => FileImportPerformanceTest.instance.Stage3TimeTotal.text = Convert.ToSingle(segTimer.Elapsed.TotalSeconds).ToString());
                //}
                LoadState = AudioDataLoadState.Loaded;
