...
namespace RuntimeAudioClipLoader.Internal
{
    ...
    /// </summary>
    public partial class AudioDataWriter : WaveStream, ISampleProvider
    {
        WaveStream writerStream; // the waveStream which we will use for all positioning
        ...
        public AudioDataWriter(Stream stream, SelectDecoder format, PreferredDecoder preferredDecoder)
        {
            writerStream = CreateWriterStream(stream, format, preferredDecoder);
            sourceBytesPerSample = (writerStream.WaveFormat.BitsPerSample / 8) * writerStream.WaveFormat.Channels;
            sampleChannel = new SampleChannel(writerStream, false);
            destBytesPerSample = 4 * sampleChannel.WaveFormat.Channels;
            length = SourceToDest(writerStream.Length);
        }
        ...
        WaveStream CreateWriterStream(Stream stream, SelectDecoder format, PreferredDecoder preferredDecoder)
        {
            ...
            switch (format)
            {
                ...
                case SelectDecoder.AutoDetect:
                    ...
                    foreach (var tryFormat in autoDetectFormatOrder)
                    {
                        ...
                        try
                        {
                            readerStream = CreateWriterStream(stream, tryFormat, preferredDecoder);
                        }
                        catch { }
        ...
        {
            get { return SourceToDest(writerStream.Position); }
            set { writerStream.Position = DestToSource(value); }
        }
